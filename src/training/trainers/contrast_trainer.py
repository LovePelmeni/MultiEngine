...
class ContrastiveTrainer(base.BaseTrainer):
    ...
    -----------
        ...
        super_epochs (int) - number of super epochs for gradient blending multimodal regularization.
    """
    def __init__(self,
        ...
        validation_dataset: datasets.ContrastiveDataset,
        batch_size: int,
        super_epochs: int,
        pair_loss_name: str,
        modal_loss_name: str,
    ...
    def train(self, train_dataset: dataset.Dataset):
        ...
        for epoch in range(self.max_epochs):
            ...
            image_train_epoch_losses = []
            desc_train_epoch_losses = []
            title_train_epoch_losses = []
            
            image_val_epoch_losses = []
            title_val_epoch_losses = []
            desc_val_epoch_losses = []
            ...
            for images, titles, descriptions, labels in tqdm(loader, desc="""
                ...
                for (
                    ...
                    # NOTE:
                    # weights are generated by gradient blending algorithm to prevent individual
                    # modalities from overfitting, while others converging during training phase
                    # original weights order:
                    #   1. image
                    #   2. title
                    #   3. description

                    img_encoder_loss = weights[0] * (img_loss + img_title_modal_sim_loss)
                    title_encoder_loss = weights[1] * (title_loss + img_title_modal_sim_loss)
                    desc_encoder_loss = weights[2] * (desc_loss + img_desc_modal_sim_loss)
                    
                    image_train_epoch_losses.append(img_encoder_loss.item())
                    title_train_epoch_losses.append(title_encoder_loss.item())
                    desc_train_epoch_losses.append(desc_encoder_loss.item())
            ...
            image_train_loss = numpy.mean(image_epoch_losses)
            title_train_loss = numpy.mean(title_epoch_losses)
            desc_train_loss = numpy.mean(desc_overall_losses)

            # TODO: create "evaluate_by_loss" method, which
            # returns individual modality losses for image, title and description data.
            (
                image_validation_loss, 
                title_validation_loss, 
                desc_validation_loss
            ) = self.evaluate_by_loss(self.validation_dataset)

            # updating Gradient Blending weights

            if (epoch + 1) % self.super_epochs == 0:
    ...
    def evaluate_by_loss(self, validation_dataset: datasets.ContrastiveDataset) -> typing.Tuple[float, float, float]:
        """
        Method evaluates multimodal 
        network on validation dataset, but instead
        of evaluation metric return losses for each modality.
        
        Parameters:
        -----------
            validation_dataset - dataset, used for evaluation
        """
        loader = self.configure_loader(dataset=validation_dataset)

        with torch.no_grad():
            image_modality_losses = []
            title_modality_losses = []
            desc_modality_losses = []
            for images, titles, descriptions, labels in loader:
        ...
        # computing validation modality losses as their average
        # across all validation batches.
        
        image_mod_loss = numpy.mean(image_modality_losses)
        title_mod_loss = numpy.mean(title_modality_losses)
        desc_mod_loss = numpy.mean(desc_modality_losses)
